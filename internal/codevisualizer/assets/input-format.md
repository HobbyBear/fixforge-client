...
多文件或多区块变更优先提供 1-3 条 `flows`，每条包含 2-8 个步骤。步骤必须按能够从代码确认的调用、数据流或状态变化顺序排列，可以跨文件；不要把文件列表或 hunk 顺序直接当成业务链路。单点局部改动或证据不足时 `flows` 可以为空。由于 unit ID 由生成器确定，无法映射的引用会被忽略；不应为了构造引用而猜测代码范围。
